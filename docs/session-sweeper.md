...
- `aborted`: `true` when a run-level circuit breaker ended the sweep early (ADR-0002 decision 4): either `transient_abort` consecutive transient failures, or deterministic failures spanning `deterministic_sessions_abort` distinct sessions. **This is the fail-loud signal for a misrouting gateway**, and nothing else in the row shows it: the run did not crash (it defended itself), so `quarantined` was deliberately zeroed by the rollback. `error` is usually NULL here too — unless the streak that tripped the `transient_abort` breaker was made of HTTP 4xx-not-429 responses (a rotated key, a renamed model), in which case `error` carries that message instead; see below. A repeating `aborted = true` with `extracted = 0` means every sweep is paying for LLM calls and storing nothing
...
- `error`: NULL when there is nothing worth surfacing here. Two things can fill it, and they are different in kind. First, the error if the **whole run** crashed (an unhandled exception out of `sweep()`). Second — since the 4xx-classification fix (ticket #17) — the message of the **last** extraction failure this run saw with `configuration_error=True`: an HTTP 4xx that is not 429 (a rotated key, a renamed or invalid model id, a malformed request; see `icarus.extraction.ExtractionFailed`). That second case does **not** mean the run crashed or aborted — extraction stays fail-open on purpose, so a config problem does not, on its own, stop the sweep or quarantine anything — it means the column would otherwise stay NULL on a run that is quietly burning every slice on the same broken credential. A single ordinary slice failure (one bad transcript, one timeout) does not land here either way — it lands in `session_extraction.error`, below
...
`tests/test_session_store.py` and `tests/test_session_lock.py` check the SQL `scripts/session_store.py` *builds*, against a `FakeConn`/`FakeCursor` pair that only proves the code sent the statement it intended to send — it agrees with whatever the code does, so it cannot check that PostgreSQL accepts `ON CONFLICT ... DO UPDATE ... WHERE`, evaluates `make_interval`/`GREATEST` the way the docstrings claim, round-trips the JSONB payload, upgrades an existing table with `ADD COLUMN IF NOT EXISTS`, that `pg_try_advisory_xact_lock` is really released by the transaction rather than the connection, or — the one the fake cannot model at all, because `FakePg.try_session_lock` just returns `True` — that the lock actually holds across `sweep()`'s production sequence (`try_session_lock` → `watermarks` → `claim`, with the transaction implicit rather than an explicit `with conn.transaction():`) and is released only once `claim()` commits, not before. `tests/test_integration_postgres.py` runs those same code paths against a real server instead.
...
Or select just this file by marker: `.venv/bin/pytest -m integration` (still needs `MEMOS_TEST_DSN`, or every test in it skips).

**Isolation.** Every test runs inside its own throwaway PostgreSQL schema, not `public`: the `conn` fixture creates `test_<random>`, points `search_path` at it before `ensure_schema` ever runs, and drops it with `CASCADE` on teardown. `scripts/session_store.py` never qualifies a schema in its SQL, so wherever `search_path` names at connect time is where `session_extraction`/`sweeper_status` land — a schema unique to the connection is what keeps that from ever being a real deployment's tables. This replaced an earlier version of this fixture that ran `DROP TABLE session_extraction, sweeper_status` unqualified against whatever database `MEMOS_TEST_DSN` pointed at: safe against a disposable container, destructive against anything real, and the old warning here said so instead of preventing it.

Sequential re-runs against the same database are **measured** safe: two runs in a row, same container, both green (see the tickets 16/17 verification). Concurrent runs are **reasoned**, not measured, safe for the tables — disjoint schemas cannot collide — but not fully: `pg_try_advisory_xact_lock` keys are cluster-wide, not schema-scoped, so two overlapping runs of this file could still contend on a lock-test session id if that id were fixed. Every lock-test session id in the file is randomised for exactly that reason, but "reasoned" is the honest word here, not "safe" — nobody has run two copies of this file at once against the same server and watched them not interfere. Pointing this at a database something else is actively using is still not recommended even with isolation: this file's own two runs are what is measured, not a third party's concurrent workload.
